...
| 스플래시 화면 | 없음 | 앱 아이콘(SVG) + 이름 | 신규 |
| 파비콘 / 홈 화면 아이콘 | 없음 | `logo.png` (PNG) | 신규 |
| 로그인 화면 히어로 | 텍스트 타이포 ("근무"/"기록") | **로고 이미지** (`logo.png`) | 변경 |
...
히어로 영역을 텍스트 타이포그래피로만 구성하고, 비밀번호 확인 필드에 대한 언급은 없었다.

### 실제 구현

**로그인 화면 히어로가 텍스트 → 로고 이미지로 변경됐다.**
...
`logo.png`를 파비콘뿐 아니라 로그인 화면 히어로에도 사용해서 브랜드 일관성을 높였다. 회원가입 모드에서는 로고 대신 "새 계정 / 만들기" 텍스트가 그대로 유지된다.

**비밀번호 확인 필드 추가**

회원가입 모드에서 `passwordConfirm` 입력 필드가 추가됐다.
...
`app.html` 명세에 파비콘, 앱 아이콘 관련 내용 없음.
...
| 파일 | 용도 |
|------|------|
| `static/app-icon.svg` | 앱 내 스플래시 화면 (`<img>` 태그로 직접 참조) |
| `static/logo.png` | 브라우저 파비콘 + 모바일 홈 화면 아이콘 |

`app.html`에 favicon과 Apple touch icon이 추가됐다:
...
SVG는 앱 내부 스플래시에 적합하고(크기 자유롭게 조정 가능), PNG는 브라우저 탭 파비콘과 iOS 홈 화면 저장 시 아이콘으로 쓰인다. 두 포맷을 역할에 맞게 분리한 구조다.
...
|------|------|-----------|
| HTML title | "근무기록" | **"공노기"** |
| 로그인 히어로 | "근무" / "기록" 텍스트 타이포 | **`logo.png` 이미지** |
| 파비콘 | 없음 | `logo.png` |
| 홈 화면 아이콘 (iOS) | 없음 | `logo.png` (apple-touch-icon) |
| 스플래시 아이콘 | 없음 | `app-icon.svg` |

"공노기"는 "공무원 노동 기록"의 줄임말로, 배포 후 앱 정체성 확립을 위해 변경됐다. 로고 이미지를 별도로 제작하면서 로그인 화면 히어로도 텍스트 타이포에서 이미지로 교체됐다.
